  ...
  return <div className="overflow-hidden rounded-3xl bg-white shadow-soft border border-slate-100">
    <table className="w-full text-sm">
      ...
      <tbody>
        ...
        >
          <td className="p-4 font-semibold">{p.partName}</td><td className="p-4 text-slate-500">{p.partCode}</td><td className="p-4 text-center">{p.model}</td><td className="p-4 text-right">Rs {Number(p.mrp).toLocaleString()}</td><td className="p-4 text-right"><span className={p.quantity <= 5 ? 'text-red-600 font-bold' : ''}>{p.quantity}</span>{salesMode && p.warehouseQuantity > 0 && <span className="block text-xs text-slate-500">WH {p.warehouseQuantity}</span>}</td>
          <td className="p-4"><div className="flex justify-end gap-2">
            ...
            {salesMode && <button disabled={availableQty <= 0} className="rounded-xl bg-brand-red px-4 py-2 text-white disabled:opacity-40" onClick={() => onAddSale(p)}>Sell</button>}
          </div></td>
        </tr>;
        })}
      </tbody>
