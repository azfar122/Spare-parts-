...
export default function Login() {
  ...
  async function submitSalesAccount(e) {
    e.preventDefault(); setError(''); setMessage('');
    setSubmitting(true);
    try {
      const res = await api.post('/auth/register-sales', { name: salesName, password: salesPassword });
      setMessage(res.data.message || 'Account request sent to admin for approval.');
      setUsername(salesName);
      setSalesName('');
      setSalesPassword('');
      setMode('login');
    } catch (err) {
      setError(err.response?.data?.message || 'Could not create sales account.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return <div className="min-h-screen grid lg:grid-cols-2 bg-white">
    ...
    <div className="grid place-items-center p-6">
      <form onSubmit={mode === 'login' ? submit : submitSalesAccount} className="w-full max-w-md rounded-3xl bg-slate-50 p-8 shadow-soft border">
        ...
        <div className="mb-5 grid grid-cols-2 gap-2 rounded-2xl bg-white p-1">
          <button type="button" onClick={() => { setMode('login'); setError(''); }} className={`rounded-xl px-3 py-2 text-sm font-bold ${mode === 'login' ? 'bg-brand-dark text-white' : 'text-slate-600 hover:bg-slate-50'}`}>Login</button>
          <button type="button" onClick={() => { setMode('register'); setError(''); setMessage(''); }} className={`rounded-xl px-3 py-2 text-sm font-bold ${mode === 'register' ? 'bg-brand-dark text-white' : 'text-slate-600 hover:bg-slate-50'}`}>Create Account</button>
        </div>
        {error && <div className="mb-4 rounded-xl bg-red-50 text-red-700 p-3 text-sm">{error}</div>}
        {message && <div className="mb-4 rounded-xl bg-emerald-50 text-emerald-700 p-3 text-sm">{message}</div>}
        {mode === 'login' ? <>
          ...
          <label className="text-sm font-medium">Salesperson Name</label><input value={salesName} onChange={e=>setSalesName(e.target.value)} placeholder="Enter your name" className="mt-2 mb-4 w-full rounded-xl border p-3" required />
          <label className="text-sm font-medium">Password</label>
          <div className="relative mt-2 mb-6">
            <input type={showSalesPassword ? "text" : "password"} value={salesPassword} onChange={e=>setSalesPassword(e.target.value)} placeholder="Create password" className="w-full rounded-xl border p-3 pr-10" minLength={6} required />
            <button type="button" onClick={() => setShowSalesPassword(!showSalesPassword)} className="absolute right-3 top-3 text-slate-500 hover:text-slate-700">{showSalesPassword ? <EyeOff size={20}/> : <Eye size={20}/>}</button>
          </div>
        </>}
        <button disabled={submitting} className="w-full rounded-xl bg-brand-red py-3 font-bold text-white hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-70 flex items-center justify-center gap-2">
          ...
          {submitting ? (mode === 'login' ? 'Logging in...' : 'Creating account...') : (mode === 'login' ? 'Login' : 'Request Approval')}
        </button>
      </form>
    </div>
  </div>;
